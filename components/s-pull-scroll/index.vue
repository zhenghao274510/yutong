<template>
	<view class="s-pull-scroll" :class="customClass">
		<scroll-view :id="scrollId" class="s-pull-scroll-view" :class="{'is-fixed':fixed}"
			:style="{'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}"
			:scroll-top="scrollTop" :scroll-with-animation="false" :scroll-y="scrollAble" :enable-back-to-top="true"
			@scroll="scroll" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"
...
<script>
	export default {
		...
		methods: {
			upx2px(value) {
				return (Array.isArray(value) ? value : [value]).map(num => uni.upx2px(Number(num || 0))).reduce((a, b) =>
						a + b) ||
					0;
			},
			// 注册列表滚动事件,用于下拉刷新
