			...
			<block v-for="(item,i) in datas" :key="i">
				<view class="State_Four">
					<view class="FourBox">
						...
						<text class="ali_top_icon" :class="[item.context.includes('签收')?'isOk':'']" style="margin: 0 0;" v-else-if="Type(item.context) == true && i!= datas.length-1">{{item.context | Type}}</text>
					...
					<view class="remark">
						...
						<text class="a14"  v-else>{{item.context}}</text>
